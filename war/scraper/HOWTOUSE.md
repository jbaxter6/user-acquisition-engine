...
Search results are saved as `.xlsx` files in `war/recruits/leads/`. Enriched opp sheets go to `war/recruits/dossiers/opp<N>/`. (Folder guide: [../fuckem/HOWTOUSE.md](../fuckem/HOWTOUSE.md#where-the-sheets-go).)
...
That's it. Files show up in `war/recruits/leads/`, and they're ready to upload.
...
fuckem's sheets (in `war/recruits/intercepts/opp<N>/`, or any sheet with Instagram/TikTok links) have no follower counts, because fuckem never opens the social profiles. With the Chrome from Step 1 open:
...
A bare file name is looked up in the opp folders under `intercepts/`; a full path works for any other sheet. It opens each Instagram/TikTok profile and writes `<same name>-enriched.xlsx` to `war/recruits/dossiers/opp<N>/` (same opp folder it came from; sheets from anywhere else go to `dossiers/` itself), with followers, following, bio, verified and the rest. Upload that one instead. YouTube/Twitch links come through without stats, and anyone whose profile won't load is kept with blank stats.
