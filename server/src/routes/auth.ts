...
export function authRouter(): Router {
  ...
  router.get("/instagram/callback", async (req, res) => {
    ...
    if (error) {
      return res.redirect(
        `${clientUrl()}/?instagram=error&reason=${encodeURIComponent(error_description ?? error)}`,
      );
    }
    if (!rawCode || !state || !pendingStates.has(state)) {
      return res.redirect(
        `${clientUrl()}/?instagram=error&reason=invalid_state`,
      );
    }
    ...
    try {
      ...
      if (!shortLivedRes.ok)
        throw new Error(`code exchange failed: ${await shortLivedRes.text()}`);
      ...
      const shortLivedToken =
        "data" in shortLivedRaw
          ? shortLivedRaw.data[0]?.access_token
      ...
      if (!normalizedToken) {
        throw new Error(
          `token exchange returned an empty access token: ${JSON.stringify({ access_token: longLivedToken })}`,
        );
      }
      ...
      const subscribeUrl = new URL(
        `https://graph.instagram.com/${GRAPH_API_VERSION}/${profile.user_id}/subscribed_apps`,
      );
      ...
      if (!subscribeRes.ok) {
        ...
        console.log(
          `Subscribed @${profile.username} to Instagram webhook messages.`,
        );
      }

      res.redirect(
        `${clientUrl()}/?instagram=connected&username=${encodeURIComponent(profile.username)}`,
      );
    } catch (err) {
      res.redirect(
        `${clientUrl()}/?instagram=error&reason=${encodeURIComponent((err as Error).message)}`,
      );
    }
  });
